...
public class BoneGen {
    ...
    private static String stringifyStream(java.io.InputStream is) {
        java.util.Scanner s = new java.util.Scanner(is).useDelimiter("\\A");
        return s.hasNext() ? s.next() : "";
    }

    /** Generate a dungeon given a TilesetType enum.
     * The main way of generating dungeons with BoneGen.
     * Consider using BoneGen.wallWrap (a static method) to surround the edges with walls.
    ...
    private int changeColor(int old_color, int num_options)
    {
        ...
        int offset = 1 + r.nextInt(num_options - 1);
